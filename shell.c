...
static int
    parse_long_options(argv, arg_start, arg_end) char** argv;
...
static int
    parse_shell_options(argv, arg_start, arg_end) char** argv;
int arg_start, arg_end;
{
  int arg_index;
...
run_startup_files()
{
  ...
  if (interactive_shell == 0 && no_rc == 0 && login_shell == 0 && act_like_sh == 0 && command_execution_string) {
...
/* Return 1 if the shell should be a restricted one based on NAME or the
   ...
   boolean value. */
int
    shell_is_restricted(name) char* name;
{
  char* temp;
...
/* Perhaps make this shell a `restricted' one, based on NAME.  If the
   ...
   started with -r. */
int
    maybe_make_restricted(name) char* name;
{
  char* temp;
...
static int
    run_one_command(command) char* command;
{
  int code;
...
static int
    bind_args(argv, arg_start, arg_end, start_index) char** argv;
...
static int
    open_shell_script(script_name) char* script_name;
{
  int fd, e, fd_is_tty;
