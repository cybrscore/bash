...
void
    dispose_fd_bitmap(fdbp) struct fd_bitmap* fdbp;
{
  FREE(fdbp->bitmap);
  free(fdbp);
}
...
void
    close_fd_bitmap(fdbp) struct fd_bitmap* fdbp;
{
  register int i;
...
{
  if (executing && showing_function_line == 0 && (variable_context == 0 || interactive_shell == 0) && currently_executing_command) {
#if defined(COND_COMMAND)
...
static int
    shell_control_structure(type) enum command_type type;
{
  switch (type) {
#if defined(ARITH_FOR_COMMAND)
  case cm_arith_for:
...
/* We can't rely on variables retaining their values across a
    ...
    if (ignore_return == 0 && invert == 0 && ((posixly_correct && interactive == 0 && special_builtin_failed) || (exit_immediately_on_error && pipe_in == NO_PIPE && pipe_out == NO_PIPE && exec_result != EXECUTION_SUCCESS))) {
      last_command_exit_value = exec_result;
      run_pending_traps();
...
static int
    mkfmt(buf, prec, lng, sec, sec_fraction) char* buf;
int prec, lng;
time_t sec;
int sec_fraction;
{
...
struct fd_bitmap* fds_to_close;
{
  ...
  subshell_level++;
  should_redir_stdin = (asynchronous && (command->flags & CMD_STDIN_REDIR) && pipe_in == NO_PIPE && stdin_redirects(command->redirects) == 0);
  ...
  if ((user_subshell || user_coproc) && (tcom->type == cm_simple || tcom->type == cm_subshell) && ((tcom->flags & CMD_TIME_PIPELINE) == 0) && ((tcom->flags & CMD_INVERT_RETURN) == 0)) {
...
static void
    cpe_dispose(cpe) struct cpelement* cpe;
{
  free(cpe);
}
...
void
    coproc_init(cp) struct coproc* cp;
{
  cp->c_name = 0;
  cp->c_pid = NO_PID;
  cp->c_rfd = cp->c_wfd = -1;
...
struct coproc*
    coproc_alloc(name, pid) char* name;
pid_t pid;
{
  struct coproc* cp;
...
static void
    coproc_free(cp) struct coproc* cp;
{
  free(cp);
}

void
    coproc_dispose(cp) struct coproc* cp;
{
  sigset_t set, oset;
...
void
    coproc_close(cp) struct coproc* cp;
{
  if (cp->c_rfd >= 0) {
    close(cp->c_rfd);
    cp->c_rfd = -1;
...
void
    coproc_rclose(cp, fd) struct coproc* cp;
int fd;
{
...
void
    coproc_wclose(cp, fd) struct coproc* cp;
int fd;
{
...
void
    coproc_checkfd(cp, fd) struct coproc* cp;
int fd;
{
  int update;
...
void
    coproc_fdclose(cp, fd) struct coproc* cp;
int fd;
{
  coproc_rclose(cp, fd);
  coproc_wclose(cp, fd);
...
void
    coproc_fdsave(cp) struct coproc* cp;
{
  cp->c_rsave = cp->c_rfd;
  cp->c_wsave = cp->c_wfd;
}
...
void
    coproc_fdrestore(cp) struct coproc* cp;
{
  cp->c_rfd = cp->c_rsave;
  cp->c_wfd = cp->c_wsave;
}
...
void
    coproc_setvars(cp) struct coproc* cp;
{
  SHELL_VAR* v;
  char *namevar, *t;
  int l;
...
void
    coproc_unsetvars(cp) struct coproc* cp;
{
  int l;
  char* namevar;
...
/* If this shell was compiled without job control support,
  ...
  case OR_OR:
    ...
    QUIT;
    if (((command->value.Connection->connector == AND_AND) && (exec_result == EXECUTION_SUCCESS)) || ((command->value.Connection->connector == OR_OR) && (exec_result != EXECUTION_SUCCESS))) {
      second = command->value.Connection->second;
...
static int
    print_index_and_element(len, ind, list) int len,
    ind;
WORD_LIST* list;
{
        ...
        COND_COM* cond;
{
  ...
  if (cond->type == COND_EXPR)
    ...
    rmatch = 0;
    patmatch = (((cond->op->word[1] == '=') && (cond->op->word[2] == '\0') && (cond->op->word[0] == '!' || cond->op->word[0] == '=')) || (cond->op->word[0] == '=' && cond->op->word[1] == '\0'));
#if defined(COND_REGEXP)
    rmatch = (cond->op->word[0] == '=' && cond->op->word[1] == '~' && cond->op->word[2] == '\0');
#endif
...
static int
    is_dirname(pathname) char* pathname;
{
  char* temp;
  int ret;
...
struct fd_bitmap* fds_to_close;
{
  ...
  if (dofork && pipe_in == NO_PIPE && pipe_out == NO_PIPE && simple_command->words && simple_command->words->word && simple_command->words->word->word && (simple_command->words->word->word[0] == '%'))
    dofork = 0;
  ...
  /* One other possibililty.  The user may want to resume an existing job.
     ...
     job. */
  if (job_control && already_forked == 0 && async == 0 && !first_word_quoted && !words->next && words->word->word[0] && !simple_command->redirects && pipe_in == NO_PIPE && pipe_out == NO_PIPE && (temp = get_string_value("auto_resume"))) {
...
run_builtin:
  ...
  if (builtin || func) {
    ...
    if (already_forked) {
      ...
      if (async) {
        if ((simple_command->flags & CMD_STDIN_REDIR) && pipe_in == NO_PIPE && (stdin_redirects(simple_command->redirects) == 0))
          async_redirect_stdin();
        setup_async_signals();
      }
...
static int
    builtin_status(result) int result;
{
  int r;
...
int flags, subshell;
{
  ...
  if (subshell == 0 && (flags & CMD_IGNORE_RETURN) && (builtin == eval_builtin || builtin == command_builtin || builtin == source_builtin)) {
...
#if defined(ARRAY_VARS)
void
    restore_funcarray_state(fa) struct func_array_state* fa;
{
  SHELL_VAR* nfv;
  ARRAY* funcname_a;
...
int cmdflags;
{
  ...
  if (pid == 0) {
    ...
    if (async) {
      if ((cmdflags & CMD_STDIN_REDIR) && pipe_in == NO_PIPE && (stdin_redirects(redirects) == 0))
        async_redirect_stdin();
      setup_async_signals();
    }
...
static char*
    getinterp(sample, sample_len, endp) char* sample;
int sample_len, *endp;
{
  register int i;
  char* execname;
...
static int
    execute_shell_script(sample, sample_len, command, args, env) char* sample;
int sample_len;
char* command;
char **args, **env;
{
...
/* Call execve (), handling interpreting shell scripts, and handling
   exec failures. */
int
    shell_execve(command, args, env) char* command;
char **args, **env;
{
  int larray, i, fd;
...
static void
    do_piping(pipe_in, pipe_out) int pipe_in,
    pipe_out;
{
  if (pipe_in != NO_PIPE) {
    if (dup2(pipe_in, 0) < 0)
