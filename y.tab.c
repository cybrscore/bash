...
void
    with_input_from_string(string, name) char* string;
const char* name;
{
  INPUT_STREAM location;
...
void
    push_stream(reset_lineno) int reset_lineno;
{
...
int
    stream_on_stack(type) enum stream_type type;
{
 ...
 */
static void
    push_string(s, expand, ap) char* s;
int expand;
alias_t* ap;
{
...
static char*
    read_a_line(remove_quoted_newline) int remove_quoted_newline;
{
...
char*
    read_secondary_line(remove_quoted_newline) int remove_quoted_newline;
{
  char* ret;
  int n, c;
...
#define push_delimiter(ds, character)                                                            \
  do {                                                                                           \
    if (ds.delimiter_depth + 2 > ds.delimiter_space)                                             \
      ds.delimiters = (char*)xrealloc(ds.delimiters, (ds.delimiter_space += 10) * sizeof(char)); \
...
static int
    shell_getc(remove_quoted_newline) int remove_quoted_newline;
{
  register int i;
  ...
  restart_read:
    ...
    if (shell_input_line) {
      ...
      if (echo_input_at_read && (shell_input_line[0] || shell_input_line_terminator != EOF) && shell_eof_token == 0)
        fprintf(stderr, "%s\n", shell_input_line);
    } else {
      shell_input_line_size = 0;
...
/* If we're at the end of an alias expansion add a space to make sure that
  ...
  if (uc == 0 && pushed_string_list && pushed_string_list->flags != PSH_SOURCE && pushed_string_list->flags != PSH_DPAREN && (parser_state & PST_COMMENT) == 0 && shell_input_line_index > 0 && shell_input_line[shell_input_line_index - 1] != ' ' && shell_input_line[shell_input_line_index - 1] != '\n' && shellmeta(shell_input_line[shell_input_line_index - 1]) == 0 && (current_delimiter(dstack) != '\'' && current_delimiter(dstack) != '"')) {
    return ' '; /* END_ALIAS */
  }
#endif
...
pop_alias:
  ...
  if
    MBTEST(uc == '\\' && remove_quoted_newline && shell_input_line[shell_input_line_index] == '\n')
    {
      if (SHOULD_PROMPT())
        prompt_again();
      line_number++;
/* What do we do here if we're expanding an alias whose definition
  ...
  if (uc == 0 && bash_input.type == st_string && *bash_input.location.string && pushed_string_list && pushed_string_list->flags == PSH_SOURCE && shell_input_line_terminator == 0) {
    shell_input_line_index = 0;
    goto restart_read;
  }
#endif
...
static void
    discard_until(character) int character;
{
  int c;
...
void
    execute_variable_command(command, vname) char *command,
    *vname;
{
  char* last_lastarg;
...
#define parsing_redirection(token) \
  (token == '<' || token == '>' || token == GREATER_GREATER || token == GREATER_BAR || token == LESS_GREATER || token == LESS_LESS_MINUS || token == LESS_LESS || token == LESS_LESS_LESS || token == LESS_AND || token == GREATER_AND || token == AND_GREATER)
...
#define command_token_position(token) \
  (((token) == ASSIGNMENT_WORD) || ((parser_state & PST_REDIRLIST) && parsing_redirection(token) == 0) || ((token) != SEMI_SEMI && (token) != SEMI_AND && (token) != SEMI_SEMI_AND && reserved_word_acceptable(token)))
...
static int
    alias_expand_token(tokstr) char* tokstr;
{
  char* expanded;
  alias_t* ap;

  if (((parser_state & PST_ALEXPNEXT) || command_token_position(last_read_token)) && (parser_state & PST_CASEPAT) == 0) {
...
static int
    special_case_tokens(tokstr) char* tokstr;
{
  ...
  else if (expecting_in_token && (last_read_token == '\n' || last_read_token == ';') && (tokstr[0] == 'd' && tokstr[1] == 'o' && tokstr[2] == '\0')) {
    expecting_in_token--;
    return (DO);
  }
...
static int
    read_token(command) int command;
{
...
re_read_token:
  ...
  if
    MBTEST(character == '#' && (!interactive || interactive_comments))
    {
  ...
  if
    MBTEST(shellmeta(character) && ((parser_state & PST_DBLPAREN) == 0))
    {
#if defined(ALIAS)
	 ...
	 layers deal with quoted newlines following the command substitution. */
      ...
      if (character == peek_char) {
        ...
        case '<':
		 ...
		 look ahead one more character. */
          ...
          else if MBTEST(peek_char == '<') return (LESS_LESS_LESS);
          else
          {
        ...
        MBTEST(character == '<' && peek_char == '&')
      return (LESS_AND);
      else if MBTEST(character == '>' && peek_char == '&') return (GREATER_AND);
      else if MBTEST(character == '<' && peek_char == '>') return (LESS_GREATER);
      else if MBTEST(character == '>' && peek_char == '|') return (GREATER_BAR);
      ...
      else if MBTEST(character == '|' && peek_char == '&') return (BAR_AND);
      else if MBTEST(character == ';' && peek_char == '&')
      {
        parser_state |= PST_CASEPAT;
#if defined(ALIAS)
	 ...
	 we will do the right thing with `{'. */
      if
        MBTEST(character == ')' && last_read_token == '(' && token_before_that == WORD)
        {
          parser_state |= PST_ALLOWOPNBRC;
#if defined(ALIAS)
          parser_state &= ~PST_ALEXPNEXT;
	 ...
	 indicates a subshell. */
      if
        MBTEST(character == '(' && (parser_state & PST_CASEPAT) == 0) /* ) */
	 ...
	 Shells running in `posix mode' don't do process substitution. */
      if
        MBTEST(posixly_correct || ((character != '>' && character != '<') || peek_char != '(')) /*)*/
  ...
  if
    MBTEST(character == '-' && (last_read_token == LESS_AND || last_read_token == GREATER_AND))
  return (character);
...
static char*
    parse_matched_pair(qc, open, close, lenp, flags) int qc; /* `"' if this construct is within double quotes */
int open, close;
int *lenp, flags;
{
  ...
  while (count) {
    ...
    /* Not exactly right yet, should handle shell metacharacters, too.  If
	 ...
	 extract_delimited_string(). */
      ...
      MBTEST((tflags & LEX_CKCOMMENT) && (tflags & LEX_INCOMMENT) == 0 && ch == '#' && (retind == 0 || ret[retind - 1] == '\n' || shellblank(ret[retind - 1])))
    tflags |= LEX_INCOMMENT;
    ...
    /* If we're reparsing the input (e.g., from parse_string_to_word_list),
	 ...
	 reparse, too. */
      ...
      MBTEST((parser_state & PST_REPARSE) && open == '\'' && (ch == CTLESC || ch == CTLNUL))
      {
        RESIZE_MALLOCED_BUFFER(ret, retind, 1, retsize, 64);
        ret[retind++] = ch;
        continue;
      ...
      MBTEST(ch == CTLESC || ch == CTLNUL) /* special shell escapes */
      {
        RESIZE_MALLOCED_BUFFER(ret, retind, 2, retsize, 64);
        ret[retind++] = CTLESC;
        ret[retind++] = ch;
      ...
      MBTEST(ch == close) /* ending delimiter */
    count--;
    ...
    {
      if
        MBTEST((flags & P_ALLOWESC) && ch == '\\')
      tflags |= LEX_PASSNEXT;
      continue;
    }

    if
      MBTEST(ch == '\\') /* backslashes */
    tflags |= LEX_PASSNEXT;
    ...
    if (flags & P_DOLBRACE) {
      /* ${param%[%]word} */
      if
        MBTEST(dolbrace_state == DOLBRACE_PARAM && ch == '%' && retind > 1)
    ...
    if
      MBTEST(posixly_correct && shell_compatibility_level > 41 && dolbrace_state != DOLBRACE_QUOTE && dolbrace_state != DOLBRACE_QUOTE2 && (flags & P_DQUOTE) && (flags & P_DOLBRACE) && ch == '\'')
    continue;
    ...
    {
      if
        MBTEST(shellquote(ch))
        {
          /* '', ``, or "" inside $(...) or other grouping construct. */
          push_delimiter(dstack, ch);
          if
            MBTEST((tflags & LEX_WASDOL) && ch == '\'') /* $'...' inside group */
          nestret = parse_matched_pair(ch, ch, ch, &nestlen, P_ALLOWESC | rflags);
          else nestret = parse_matched_pair(ch, ch, ch, &nestlen, rflags);
          pop_delimiter(dstack);
          CHECK_NESTRET_ERROR();

          if
            MBTEST((tflags & LEX_WASDOL) && ch == '\'' && (extended_quote || (rflags & P_DQUOTE) == 0))
            {
              /* Translate $'...' here. */
    ...
    /* XXX - sh and ksh93 don't do this - XXX */
      ...
      MBTEST(open != '`' && (tflags & LEX_WASDOL) && (ch == '(' || ch == '{' || ch == '[')) /* ) } ] */
...
#if defined(PROCESS_SUBSTITUTION)
    if
      MBTEST((ch == '<' || ch == '>') && (tflags & LEX_GTLT) == 0)
    tflags |= LEX_GTLT;
    else tflags &= ~LEX_GTLT;
#endif
    if
      MBTEST(ch == '$' && (tflags & LEX_WASDOL) == 0)
    tflags |= LEX_WASDOL;
    else tflags &= ~LEX_WASDOL;
  }
...
static char*
    parse_comsub(qc, open, close, lenp, flags) int qc; /* `"' if this construct is within double quotes */
int open, close;
int *lenp, flags;
{
  ...
  comsub_readchar:
    ...
    /* Skip whitespace */
    if
      MBTEST(shellblank(ch) && (tflags & LEX_HEREDELIM) == 0 && lex_rwlen == 0)
      {
        /* Add this character. */
        RESIZE_MALLOCED_BUFFER(ret, retind, 1, retsize, 64);
        ret[retind++] = ch;
    ...
    if
      MBTEST((tflags & LEX_RESWDOK) == 0 && (tflags & LEX_CKCASE) && (tflags & LEX_INCOMMENT) == 0 && (shellmeta(ch) || ch == '\n'))
      {
        /* Add this character. */
        RESIZE_MALLOCED_BUFFER(ret, retind, 1, retsize, 64);
        ret[retind++] = ch;
    ...
    if (tflags & LEX_RESWDOK) {
      if
        MBTEST(islower((unsigned char)ch))
        {
          /* Add this character. */
          RESIZE_MALLOCED_BUFFER(ret, retind, 1, retsize, 64);
          ret[retind++] = ch;
        ...
        MBTEST(lex_rwlen == 4 && shellbreak(ch))
        {
          if (STREQN(ret + retind - 4, "case", 4)) {
            ...
            tflags |= LEX_RESWDOK;
            lex_rwlen = 0;
          } else if (STREQN(ret + retind - 4, "done", 4) || STREQN(ret + retind - 4, "then", 4) || STREQN(ret + retind - 4, "else", 4) || STREQN(ret + retind - 4, "elif", 4) || STREQN(ret + retind - 4, "time", 4)) {
        ...
        MBTEST((tflags & LEX_CKCOMMENT) && ch == '#' && (lex_rwlen == 0 || ((tflags & LEX_INWORD) && lex_wlen == 0))); /* don't modify LEX_RESWDOK if we're starting a comment */
      /* Allow `do' followed by space, tab, or newline to preserve the
        ...
        MBTEST(((tflags & LEX_INCASE) == 0) && (isblank((unsigned char)ch) || ch == '\n') && lex_rwlen == 2 && STREQN(ret + retind - 2, "do", 2))
        {
          /*itrace("parse_comsub:%d: lex_incase == 0 found `%c', found \"do\"", line_number, ch);*/
          lex_rwlen = 0;
        }
      else if
        MBTEST((tflags & LEX_INCASE) && ch != '\n')
...
/*itrace("parse_comsub:%d: found `%c', lex_reswordok -> 0", line_number, ch);*/
    ...
    if
      MBTEST((tflags & LEX_INCOMMENT) == 0 && (tflags & LEX_CKCASE) && ch == '<')
      {
        /* Add this character. */
        RESIZE_MALLOCED_BUFFER(ret, retind, 1, retsize, 64);
        ret[retind++] = ch;
      ...
      MBTEST((tflags & LEX_CKCOMMENT) && (tflags & LEX_INCOMMENT) == 0 && ch == '#' && (((tflags & LEX_RESWDOK) && lex_rwlen == 0) || ((tflags & LEX_INWORD) && lex_wlen == 0)))
      {
    ...
    if
      ...
      MBTEST(ch == close && (tflags & LEX_INCASE) == 0) /* ending delimiter */
      {
        count--;
        /*itrace("parse_comsub:%d: found close: count = %d", line_number, count);*/
      }
    else if
      MBTEST(((flags & P_FIRSTCLOSE) == 0) && (tflags & LEX_INCASE) == 0 && ch == open) /* nested begin */
      {
        count++;
    ...
    if
      ...
      MBTEST((tflags & LEX_WASDOL) && (ch == '(' || ch == '{' || ch == '[')) /* ) } ] */
...
char*
    xparse_dolparen(base, string, indp, flags) char* base;
char* string;
int* indp;
int flags;
{
...
static int
    parse_arith_cmd(ep, adddq) char** ep;
int adddq;
{
...
static int
    token_is_assignment(t, i) char* t;
int i;
{
  int r;
  char* atoken;
...
static int
    read_token_word(character) int character;
{
  ...
  for (;;) {
    ...
    if
      MBTEST(character == '\\')
      {
        ...
        if (peek_char == '\n') {
          ...
          if (cd == 0 || cd == '`' || (cd == '"' && peek_char >= 0 && (sh_syntaxtab[peek_char] & CBSDQUOTE)))
            pass_next_character++;
    ...
    if
      MBTEST((parser_state & PST_REGEXP) && (character == '(' || character == '|')) /*)*/
      {
        if (character == '|')
          goto got_character;
    ...
    if
      MBTEST(extended_glob && PATTERN_CHAR(character))
      {
        peek_char = shell_getc(1);
        if
          MBTEST(peek_char == '(') /* ) */
    ...
    if (shellexp(character)) {
      ...
      /* $(...), <(...), >(...), $((...)), ${...}, and $[...] constructs */
      if
        MBTEST(peek_char == '(' || ((peek_char == '{' || peek_char == '[') && character == '$')) /* ) ] } */
        {
          if (peek_char == '{') /* } */
      ...
      /* This handles $'...' and $"..." new-style quoted strings. */
        ...
        MBTEST(character == '$' && (peek_char == '\'' || peek_char == '"'))
        {
          int first_line;
    ...
    /* Identify possible array subscript assignment; match [...].  If
	 ...
	 `sub' as if it were enclosed in double quotes. */
      ...
      MBTEST(character == '[' && /* ] */
          ((token_index > 0 && assignment_acceptable(last_read_token) && token_is_ident(token, token_index)) || (token_index == 0 && (parser_state & PST_COMPASSIGN))))
      {
    ...
    /* Identify possible compound array variable assignment. */
      ...
      MBTEST(character == '=' && token_index > 0 && (assignment_acceptable(last_read_token) || (parser_state & PST_ASSIGNOK)) && token_is_assignment(token, token_index))
      {
        peek_char = shell_getc(1);
        if
          MBTEST(peek_char == '(') /* ) */
...
got_token:
  ...
  if
    MBTEST(all_digit_token && (character == '<' || character == '>' || last_read_token == LESS_AND || last_read_token == GREATER_AND))
    {
  ...
  if (token[0] == '{' && token[token_index - 1] == '}' && (character == '<' || character == '>')) {
...
static int
    reserved_word_acceptable(toksym) int toksym;
{
  switch (toksym) {
  case '\n':
  case ';':
...
int
    find_reserved_word(tokstr) char* tokstr;
{
  int i;
...
void
    set_current_prompt_level(x) int x;
{
...
static int
    prompt_history_number(pmt) char* pmt;
{
  int ret;
...
char*
    decode_prompt_string(string) char* string;
{
  WORD_LIST* list;
...
static char*
    error_token_from_token(tok) int tok;
{
  char* t;
...
static void
    report_syntax_error(message) char* message;
{
  char *msg, *p;
...
static void
    discard_parser_constructs(error_p) int error_p;
{
}
...
WORD_LIST*
    parse_string_to_word_list(s, flags, whom) char* s;
int flags;
const char* whom;
{
  WORD_LIST* wl;
...
static char*
    parse_compound_assignment(retlenp) int* retlenp;
{
  WORD_LIST *wl, *rl;
