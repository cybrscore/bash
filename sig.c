...
void
    initialize_signals(reinit) int reinit;
{
...
void
    jump_to_top_level(value) int value;
{
...
#if defined(SIGWINCH)
sighandler
    sigwinch_sighandler(sig) int sig;
{
#if defined(MUST_REINSTALL_SIGHANDLERS)
...
sighandler
    sigterm_sighandler(sig) int sig;
{
...
SigHandler*
    set_signal_handler(sig, handler) int sig;
SigHandler* handler;
{
  struct sigaction act, oact;
