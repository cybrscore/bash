...
/* Is the history expansion starting at string[i] one that should not
   be expanded? */
static int
    bash_history_inhibit_expansion(string, i) char* string;
int i;
{
  int t, si;
  char hx[2];
...
void
    bash_history_reinit(interact) int interact;
{
#if defined(BANG_HISTORY)
...
int
    bash_delete_history_range(first, last) int first,
    last;
{
  register int i;
...
int
    maybe_append_history(filename) char* filename;
{
  int fd, result;
  struct stat buf;
...
static int
    history_expansion_p(line) char* line;
{
  register char* s;
...
char*
    pre_process_line(line, print_changes, addit) char* line;
...
static char*
    filter_comments(line) char* line;
{
  char* p;
...
static int
    check_history_control(line) char* line;
{
  HIST_ENTRY* temp;
  int r;
...
/* Add LINE to the history list, handling possibly multi-line compound
   ...
   constructs. */
void
    maybe_add_history(line) char* line;
{
  int is_comment;
  ...
  if (current_command_line_count > 1) {
    if (current_command_first_line_saved && ((parser_state & PST_HEREDOC) || literal_history || dstack.delimiter_depth != 0 || is_comment != 1))
...
int
    check_add_history(line, force) char* line;
int force;
{
#if defined(AUDIT_BASH)
  audit_cmd(line);
...
void
    bash_add_history(line) char* line;
{
  ...
  if (command_oriented_history && current_command_line_count > 1) {
    ...
    if (current) {
      ...
      if (dstack.delimiter_depth == 0 && current->line[curlen - 1] == '\\' && current->line[curlen - 2] != '\\') {
...
static void
    really_add_history(line) char* line;
{
...
static int
    histignore_item_func(ign) struct ign* ign;
{
...
void
    setup_history_ignore(varname) char* varname;
{
...
static char*
    expand_histignore_pattern(pat) char* pat;
{
  HIST_ENTRY* phe;
  char* ret;
...
static int
    history_should_ignore(line) char* line;
{
