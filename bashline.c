...
void
    posix_readline_initialize(on_or_off) int on_or_off;
{
  if (on_or_off)
...
int
    enable_hostname_completion(on_or_off) int on_or_off;
{
  int old_value;
  char *at, *nv, *nval;
  ...
  if (bash_readline_initialized == 0 && (rl_completer_word_break_characters == 0 || rl_completer_word_break_characters == rl_basic_word_break_characters)) {
    if (on_or_off)
...
static int
    display_shell_version(count, c) int count,
    c;
{
  rl_crlf();
...
static void
    snarf_hosts_from_file(filename) char* filename;
{
  FILE* file;
...
static char**
    hostnames_matching(text) char* text;
{
...
set_saved_history()
{
  /* XXX - compensate for assumption that history was `shuffled' if it was
     actually not. */
  if (HISTORY_FULL() && hist_last_line_added == 0 && saved_history_line_to_use < history_length - 1)
...
static int
    operate_and_get_next(count, c) int count,
    c;
{
  int where;
...
static int
    edit_and_execute_command(count, c, editing_mode, edit_command) int count,
    c, editing_mode;
char* edit_command;
{
...
#if defined(VI_MODE)
static int
    vi_edit_and_execute_command(count, c) int count,
    c;
{
  if (posixly_correct)
...
static int
    emacs_edit_and_execute_command(count, c) int count,
    c;
{
...
#if defined(ALIAS)
static int
    posix_edit_macros(count, key) int count,
    key;
{
  int c;
...
static int
    bash_forward_shellword(count, key) int count,
    key;
{
  size_t slen;
  int sindex, c, p;
...
static int
    bash_backward_shellword(count, key) int count,
    key;
{
  size_t slen;
  int sindex, c, p;
...
static int
    bash_kill_shellword(count, key) int count,
    key;
{
  int p;
...
static int
    bash_backward_kill_shellword(count, key) int count,
    key;
{
  int p;
 ...
 */
static int
    find_cmd_start(start) int start;
{
  ...
  while (((s = skip_to_delim(rl_line_buffer, os, COMMAND_SEPARATORS, SD_NOJMP | SD_COMPLETE /*|SD_NOSKIPCMD*/)) <= start) && rl_line_buffer[s]) {
...
static char*
    find_cmd_name(start, sp, ep) int start;
int *sp, *ep;
{
  char* name;
  register int s, e;
...
int start, end;
{
  ...
  if (in_command_position && ti >= 0 && rl_line_buffer[ti] == '`' && *text != '`' && unclosed_pair(rl_line_buffer, end, "`") == 0)
    in_command_position = 0;
  ...
  if (*text == '`' && rl_completion_quote_character != '\'' && (in_command_position || (unclosed_pair(rl_line_buffer, start, "`") && unclosed_pair(rl_line_buffer, end, "`"))))
    matches = rl_completion_matches(text, command_subst_completion_function);

#if defined(PROGRAMMABLE_COMPLETION)
  /* Attempt programmable completion. */
  have_progcomps = prog_completion_enabled && (progcomp_size() > 0);
  if (matches == 0 && (in_command_position == 0 || text[0] == '\0') && current_prompt_string == ps1_prompt) {
    ...
    else if (s >= e && n[0] == '\0' && text[0] == '\0' && start > 0 && was_assignment == 0 && member(rl_line_buffer[start - 1], COMMAND_SEPARATORS)) {
      foundcs = 0;
...
static int
    bash_command_name_stat_hook(name) char** name;
{
  char *cname, *result;
...
static char*
    history_expand_line_internal(line) char* line;
{
  char* new_line;
  int old_verify;
...
static void
    maybe_make_readline_line(new_line) char* new_line;
{
...
static void
    set_up_new_line(new_line) char* new_line;
{
...
static int
    alias_expand_line(count, ignore) int count,
    ignore;
{
  char* new_line;
...
static int
    history_expand_line(count, ignore) int count,
    ignore;
{
  char* new_line;
...
static int
    tcsh_magic_space(count, ignore) int count,
    ignore;
{
...
static int
    history_and_alias_expand_line(count, ignore) int count,
    ignore;
{
  char* new_line;
...
/* History and alias expand the line, then perform the shell word
   ...
   set of operations. */
static int
    shell_expand_line(count, ignore) int count,
    ignore;
{
  char* new_line;
...
static void
    _ignore_completion_names(names, name_func) char** names;
...
static int
    filename_completion_ignore(names) char** names;
{
#if 0
...
static int
    bash_ignore_filenames(names) char** names;
{
...
static int
    bash_ignore_everything(names) char** names;
{
...
static char *
    restore_tilde(val, directory_part) char *val,
    *directory_part;
{
  int l, vl, dl2, xl;
  char *dh2, *expdir, *ret;
...
static char *
    maybe_restore_tilde(val, directory_part) char *val,
    *directory_part;
{
  rl_icppfunc_t* save;
  char* ret;
...
static void
    bash_directory_expansion(dirname) char** dirname;
{
  char *d, *nd;
...
static char*
    bash_filename_rewrite_hook(fname, fnlen) char* fname;
int fnlen;
{
  char* conv;
...
static int
    bash_filename_stat_hook(dirname) char** dirname;
{
...
static int
    bash_directory_completion_hook(dirname) char** dirname;
{
  char *local_dirname, *new_dirname, *t;
...
static int
    dynamic_complete_history(count, key) int count,
    key;
{
  int r;
...
static int
    bash_dabbrev_expand(count, key) int count,
    key;
{
...
#if defined(SPECIFIC_COMPLETION_FUNCTIONS)
static int
    bash_complete_username(ignore, ignore2) int ignore,
    ignore2;
{
...
static int
    bash_possible_username_completions(ignore, ignore2) int ignore,
    ignore2;
{
...
static int
    bash_complete_username_internal(what_to_do) int what_to_do;
{
  return bash_specific_completion(what_to_do, rl_username_completion_function);
}

static int
    bash_complete_filename(ignore, ignore2) int ignore,
    ignore2;
{
...
static int
    bash_possible_filename_completions(ignore, ignore2) int ignore,
    ignore2;
{
...
static int
    bash_complete_filename_internal(what_to_do) int what_to_do;
{
  rl_compentry_func_t* orig_func;
  rl_completion_func_t* orig_attempt_func;
...
static int
    bash_complete_hostname(ignore, ignore2) int ignore,
    ignore2;
{
...
static int
    bash_possible_hostname_completions(ignore, ignore2) int ignore,
    ignore2;
{
...
static int
    bash_complete_variable(ignore, ignore2) int ignore,
    ignore2;
{
...
static int
    bash_possible_variable_completions(ignore, ignore2) int ignore,
    ignore2;
{
...
static int
    bash_complete_command(ignore, ignore2) int ignore,
    ignore2;
{
...
static int
    bash_possible_command_completions(ignore, ignore2) int ignore,
    ignore2;
{
...
static int
    bash_complete_hostname_internal(what_to_do) int what_to_do;
{
  return bash_specific_completion(what_to_do, hostname_completion_function);
}

static int
    bash_complete_variable_internal(what_to_do) int what_to_do;
{
  return bash_specific_completion(what_to_do, variable_completion_function);
}

static int
    bash_complete_command_internal(what_to_do) int what_to_do;
{
...
static int
    bash_glob_completion_internal(what_to_do) int what_to_do;
{
...
static char*
    bash_glob_quote_filename(s, rtype, qcp) char* s;
int rtype;
char* qcp;
{
...
static int
    bash_glob_complete_word(count, key) int count,
    key;
{
  int r;
...
static int
    bash_glob_expand_word(count, key) int count,
    key;
{
...
static int
    bash_glob_list_expansions(count, key) int count,
    key;
{
  return bash_glob_completion_internal('?');
}
...
static int
    bash_specific_completion(what_to_do, generator) int what_to_do;
...
static int
    bash_vi_complete(count, key) int count,
    key;
{
#if defined(SPECIFIC_COMPLETION_FUNCTIONS)
  int p, r;
...
static char*
    bash_dequote_filename(text, quote_char) char* text;
int quote_char;
{
  char *ret, *p, *r;
  int l, quoted;
...
static char*
    quote_word_break_chars(text) char* text;
{
  char *ret, *r, *s;
  int l;
...
/* Quote a filename using double quotes, single quotes, or backslashes
   ...
   memory. */
static char*
    bash_quote_filename(s, rtype, qcp) char* s;
int rtype;
char* qcp;
{
...
static int
    bash_execute_unix_command(count, key) int count; /* ignored */
int key;
{
...
static int
    isolate_sequence(string, ind, need_dquote, startp) char* string;
...
int
    bind_keyseq_to_unix_command(line) char* line;
{
  Keymap kmap;
  char *kseq, *value;
  int i, kstart;
